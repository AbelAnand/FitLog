...
export function ProgressChart({ points, unit }: { points: ChartPoint[]; unit: Unit }) {
    ...
    <div className="h-[220px] -mx-2">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={points} margin={{ top: 16, right: 20, bottom: 4, left: 0 }}>
          ...
          />
          <YAxis
            domain={[Math.floor((min - pad) / 5) * 5, Math.ceil((max + pad) / 5) * 5]}
            tick={{ fill: MUTED, fontSize: 11 }}
            axisLine={false}
            tickLine={false}
            width={40}
              ...
              return (
                <div className="rounded-xl bg-surface-3 border border-border px-3 py-2 text-[13px] shadow-lg">
                  ...
                  <div className="font-semibold tabular">
                    {formatWeight(p.weight)} {unit} × {p.reps}
